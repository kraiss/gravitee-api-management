...
public class HttpPolicyChain extends AbstractPolicyChain<HttpPolicy> implements Hookable<HttpHook> {
    ...
    protected Completable executePolicy(final BaseExecutionContext baseCtx, final HttpPolicy policy) {
        ...
        return switch (phase) {
            ...
            case MESSAGE_REQUEST -> HookHelper.hook(() -> policy.onMessageRequest(ctx), policy.id(), messageHooks, ctx, phase);
            case MESSAGE_RESPONSE -> HookHelper.hook(() -> policy.onMessageResponse(ctx), policy.id(), messageHooks, ctx, phase);
