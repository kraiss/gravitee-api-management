...
public class DefaultHttpRequestDispatcher implements HttpRequestDispatcher {
    ...
    public Completable dispatch(HttpServerRequest httpServerRequest, String serverId) {
        ...
        if (httpAcceptor == null || httpAcceptor.reactor() == null) {
            ...
            List<TracingHook> notFoundProcessorHook = gatewayTracingContext.isVerbose() ? List.of(tracingHook) : List.of();
            return HookHelper
                .hook(
                ...
                .andThen(handleNotFound(mutableCtx, notFoundProcessorHook))
            ...
            ProcessorChain preProcessorChain = platformProcessorChainFactory.preProcessorChain();
            List<TracingHook> processHooks = tracingContext.isVerbose() ? List.of(tracingHook) : List.of();
            return HookHelper
                .hook(
                    () -> preProcessorChain.execute(mutableCtx, ExecutionPhase.REQUEST),
                    preProcessorChain.getId(),
    ...
    private Completable handleNotFound(final MutableExecutionContext ctx, final List<TracingHook> notFoundProcessorHook) {
